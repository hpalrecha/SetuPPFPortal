...
export function EditPartnerModal({
  ...
  const { data: partnerData } = useQuery<{ showroomIds?: string[] }>({
    queryKey: ['/api/partners', partner?.id],
    enabled: open && isEditing && !!partner?.id,
  });
  ...
  useEffect(() => {
    if (partner && open) {
      form.reset({
        ...
        showroomIds: partnerData?.showroomIds || [],
      });
  ...
  }, [partner, open, form, partnerCategories, partnerData]);
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Showroom Assignment */}
            <div className="space-y-4">
              <h3 className="text-lg font-medium">Showroom Assignment</h3>
              <p className="text-sm text-muted-foreground">
                Select the showrooms this partner can serve (at least one required)
              </p>
