...
export default function WorkOrdersPage() {
  ...
  // Pagination state for client-side filtering
  ...
  return (
    ...
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-4">
            <div className="flex items-center gap-4">
              <div className="text-sm text-muted-foreground">
                Showing {startIndex + 1}-{Math.min(endIndex, sortedWorkOrders.length)} of {sortedWorkOrders.length} work orders
                {sortedWorkOrders.length < allWorkOrders.length && ` (filtered from ${allWorkOrders.length})`}
              </div>
            </div>
      ...
      {!isLoading && sortedWorkOrders.length === 0 && allWorkOrders.length > 0 && (
        <Card>
          <CardContent className="pt-6">
      ...
      {totalPages > 1 && workOrders.length > 0 && (
