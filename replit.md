...
- **Job Card System**: Real-time status tracking, SLA monitoring, and notifications.
  ...
  - **Mobile Image Upload Optimization** (Dec 2025): Added client-side image processing for mobile browser compatibility. Features include: HEIC/HEIF format conversion (iOS photos), automatic image compression to max 1MB, progress indicators during upload, and better error handling. Uses `browser-image-compression` and `heic2any` libraries. Processing happens before upload to reduce file sizes and ensure compatibility across all browsers.
  ...
  - **Signed URLs for Media** (Dec 2025): Job card media (pre-installation photos) stored with private ACLs now use signed URLs with 1-hour TTL for secure viewing. This fixes broken image issues when viewing job card details.
  - **Direct Job Card Links** (Dec 2025): Added `/job-cards/:id` route for direct deep linking from WhatsApp notifications. Partners can now click job card links and be taken directly to the job card detail view.
  - **Accounts Team Invoice Notifications** (Dec 2025): When a job card is approved and moves to PENDING_SALES_INVOICE status, automated email notifications are sent to the accounts team (accounts@plusnineoneinc.in, hp@justsigns.co.in, nidhi@justsigns.co.in). The email includes comprehensive job card details: customer info (name, phone, email, address), vehicle info (brand, model, reg no, color), service details, partner info, organization hierarchy (OEM, dealership, showroom), invoice amount, billing info (bill from/to), and a direct link to view the full job card.
- **Partner Application Notification Emails** (Apr 2026): Configurable list of email addresses that receive a notification whenever a new partner application is activated via the Pulse webhook. Managed via a `system_settings` table (key/value JSONB). SUPER_ADMIN and ADMIN can configure the list in Settings → "Partner Application Notifications" card. API: `GET/PUT /api/system-settings/:key`. Webhook service reads the setting and sends HTML notification emails using the existing email service.
...
- **Pricing Engine**: Hierarchical pricing rules applied at OEM, Dealership, and Partner levels.
  ...
  - **Payout Recalculation** (Dec 2025): Added `POST /api/payouts/:id/recalculate` endpoint to fix payouts created with ₹0.00 before pricing rules were established. Also added `POST /api/payouts/bulk-recalculate` (SUPER_ADMIN only) for batch recalculation of all pending_review payouts with zero amounts. Recalculation uses DETAILER_PRICING rules matching partner + service category + vehicle model.
...
- **Hierarchical Permission System** (Nov 2025): Added ADMIN and MANAGER roles:
  ...
  - **MANAGER Role**: State-based READ-ONLY access with selective creation rights
    - View: Can see ALL OEMs (no state filtering), but dealerships, showrooms, work orders, partners, sales persons, allocations, and job cards are filtered by allowedStates
    - Filtering Hierarchy: OEMs (ALL visible) → Dealerships (state-filtered) → Showrooms (state-filtered via dealership) → Partners (state-filtered) → Allocations (state-filtered) → Job Cards (state-filtered via work order's dealership)
    ...
    - Restrictions: Cannot create partners or allocations; cannot create/edit dealerships or showrooms; cannot access entities outside allowedStates; no delete permissions; no access to payout settlement page
    ...
    - Backend: Removed from POST /api/partners and POST /api/allocations endpoints; retained in POST /api/work-orders with state validation; job cards filtered by state in GET /api/job-cards and GET /api/job-cards/:id
